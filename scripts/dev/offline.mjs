...
// Every phase runs inside this try. They are top-level awaits, so without it
// the first rejection anywhere below skips both the summary — losing the PASS
// lines already earned — and the shutdown, and this driver owns a browser and
// a listening socket rather than borrowing them.
try {
  ...
  // Every lookup is guarded and the whole thing has a fallback: on a page where
  // nothing booted, `#scene` is absent and an unguarded `.width` throws inside
  // evaluate, killing the run at exactly the point a readable FAIL line is most
  // useful.
  ...
  }).catch(() => ({ layers: 0, speeds: 0, buffer: 'evaluate failed' }));
  check(
  ...
  // exitCode rather than exit(): a thrown error still needs to surface with
  // its stack, which process.exit() inside a finally would swallow.
